    ...
    <div style={{ minHeight:"100vh", background:"#F5F6FA", color:"#1a1a1a", padding:24 }}>
      ...
      <div style={{ maxWidth:960, margin:"0 auto" }}>
                  ...
                  {tableData.map((visit) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding:"12px 16px", fontSize:11, color:"#185FA5", fontFamily:"monospace", whiteSpace:"nowrap" }}>
                          {visit.startTime ? new Date(visit.startTime).toLocaleString() : "—"}
                        </td>
                        <td style={{ padding:"12px 16px", fontSize:11, color: visit.endTime ? "#A32D2D" : "#B4B2A9", fontFamily:"monospace", whiteSpace:"nowrap" }}>
        ...
        <div style={{ maxWidth:420, margin:"0 auto", background:"#fff", border:"0.5px solid #D3D1C7", borderRadius:16, padding:28 }}>
          ...
          {step === "otp" && (
            <div>
              <p style={{ fontSize:14, fontWeight:600, marginBottom:4, color:"#1a1a1a" }}>Verify with OTP</p>
              <p style={{ fontSize:12, color:"#888780", marginBottom:18 }}>Ask the doctor for the 6-digit code sent to their number</p>

              <div style={{ display:"flex", alignItems:"center", gap:12, borderRadius:10, padding:"12px 14px", marginBottom:18, background:"#FAEEDA", border:"0.5px solid #EF9F27" }}>
                <span style={{ fontSize:18 }}>📱</span>
                <div>
                  <p style={{ fontSize:13, fontWeight:600, color:"#633806" }}>OTP sent to doctor's number</p>
                  <p style={{ fontSize:12, marginTop:2, color:"#888780" }}>+91 {doctorPhone}</p>
                </div>
              </div>

              <label style={{ display:"block", fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:".1em", color:"#888780", marginBottom:6 }}>Enter OTP</label>
              <input
                type="number"
                style={{ ...inp, marginBottom:14, textAlign:"center", fontSize:22, letterSpacing:".5em", fontFamily:"monospace" }}
                placeholder="••••••"
                value={otp}
                onChange={e => setOtp(e.target.value)}
              />

              <label style={{ display:"block", fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:".1em", color:"#888780", marginBottom:6 }}>Meeting feedback</label>
              <textarea
                style={{ ...inp, marginBottom:18, resize:"none", minHeight:80 }}
                placeholder="Write your meeting notes here…"
                value={feedback}
                onChange={e => setFeedback(e.target.value)}
              />

              <button style={btnSuccess} onClick={verifyOtp} disabled={loading}>
