...
export default function PerformanceManager() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        const filtered = list.filter((t) => String(t.manager_id || '') === myId);
  ...
  const isMeSelected = useMemo(() => {
    return String(selectedEmployeeId || '') === String(myId || '');
  }, [selectedEmployeeId, myId]);
  ...
  // ✅ EDIT MY RECORD (prefill)
  const openEditMyForm = () => {
    if (!isMeSelected || !record) {
      setShowMyForm(true);
      return;
    }

    if (tab === 'daily') {
      setMyWhatDoneToday(String(record?.whatDoneToday || ''));
      setMyWhatCompletedToday(String(record?.whatCompletedToday || ''));
    } else {
      setMyPlanForThisMonth(String(record?.planForThisMonth || ''));
      setMyCompletedThisMonth(String(record?.completedThisMonth || ''));
    }
  ...
  const canEditMyRecord = useMemo(() => isMeSelected && !!record, [isMeSelected, record]);
              ...
              <Stack direction="row" gap={1} flexWrap="wrap" justifyContent="flex-end">
                {canEditMyRecord ? (
                  <Button
                    ...
                    variant="outlined"
                    startIcon={<EditIcon />}
                    onClick={openEditMyForm}
                    sx={{ borderRadius: 999, fontWeight: 900, textTransform: 'none' }}
                  >
                    {tab === 'daily' ? 'Edit My Daily' : 'Edit My Monthly'}
                  </Button>
                ) : null}
                      ...
                      >
                        ...
                        {!hasSavedReview ? (
                          isMeSelected ? (
                            <Chip
                              icon={<LockRoundedIcon />}
                              label="You can't review yourself"
                              size="small"
                              sx={{ fontWeight: 900 }}
                              variant="outlined"
                            />
