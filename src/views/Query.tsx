...
const Query = () => {
  ...
  // 🔹 Replies dialog open handler
  const handleViewReplies = (query: any) => {
    setReplyDialogQuery(query);
        ...
        renderCell: params => {
          ...
          return (
            <Button
              ...
              size="small"
              disabled={count === 0}
              onClick={() => handleViewReplies(params.row)}
              sx={{ textTransform: 'none', borderRadius: 6, fontSize: 12 }}
            >
              {count === 0 ? 'No Replies' : `Reply (${count})`}
            </Button>
          );
        },
      },
